...
def filter_over_Y(image, kernel):
  ...
  padded_image = np.zeros(shape=(shape_of_image[0], shape_of_image[1] + kernel.size, shape_of_image[2]))
...
def filter_over_X(image, kernel):
  filtered_image = np.zeros_like(image).astype(np.float32)
  shape_of_image = np.shape(image)
  padded_image = np.zeros(shape=(shape_of_image[0], shape_of_image[1], shape_of_image[2] + kernel.size))
  padded_image[:, :, kernel.size//2:shape_of_image[2] + kernel.size//2] = image
  X_dim = image.shape[2]
