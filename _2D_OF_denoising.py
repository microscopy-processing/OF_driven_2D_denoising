...
def get_flow(reference_line, target_line, l, w):
  ...
  zero_line = np.zeros_like(reference_line)
  reference = np.stack([*[zero_line for i in range(N//2)], reference_line, *[zero_line for i in range(N//2)]])
  #reference = np.stack([reference_line for i in range(N)])
  #reference = np.stack([np.roll(reference_line,i) for i in range(N)])
  #reference = np.stack([reference_line, reference_line, reference_line])
  print("get_flow: reference =", reference[1,...][0:10])
  target = np.stack([*[zero_line for i in range(N//2)], target_line, *[zero_line for i in range(N//2)]])
  ...
  #flow = cv2.calcOpticalFlowFarneback(prev=target, next=target, flow=None, pyr_scale=0.5, levels=l, winsize=w, iterations=3, poly_n=5, poly_sigma=1.2, flags=0)
